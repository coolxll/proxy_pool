...
class TestExport:
    ...
    def test_export_returns_plain_text(self, client, mocks):
        mocks["getAll"].return_value = [
            Proxy("1.2.3.4:8080", source="test"),
            Proxy("5.6.7.8:443", source="test", https=True),
        ]
        ...
        assert resp.get_data(as_text=True) == "1.2.3.4:8080\n5.6.7.8:443\n"

    def test_export_passes_https_filter(self, client, mocks):
        mocks["getAll"].return_value = []

        client.get("/export/?type=https")

        mocks["getAll"].assert_called_with(True)
