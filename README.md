...
> 本地整合说明：工作区中的 `ProxyPool`、`proxySpeedTest` 和 `ProxyTester` 已收敛到本项目。抓取、校验、Redis 和 API 只保留一套；导出、目标站点验证和测速由统一 CLI 提供。详见 [docs/migration.md](docs/migration.md)。
...
| /export | GET | 纯文本导出代理，每行一个 |可选参数: `?type=https` 过滤支持https的代理|
